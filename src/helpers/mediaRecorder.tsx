import { createSignal, createEffect, JSX, Accessor, onCleanup } from "solid-js"
...
export type MediaRecorderRenderProps = {
	...
	stopRecording: () => void
	mediaBlobUrl: Accessor<string>
	mediaChunks: Accessor<Blob[]>
	mediaStream: Accessor<MediaStream | undefined>
...
export type MediaRecorderProps = MediaRecorderHookProps & {
	render: (props: MediaRecorderRenderProps) => JSX.Element
}
...
export function useSolidMediaRecorder({
	audio = true,
	video = false,
	onStop = () => null,
	blobPropertyBag,
	...
	const [mediaRecorder, setMediaRecorder] = createSignal<
		MediaRecorder | undefined
	>(undefined)
	const [mediaChunks, setMediaChunks] = createSignal<Blob[]>([])
	const [mediaStream, setMediaStream] = createSignal<MediaStream | undefined>(
		undefined
	)
	...
	return {
		...
		previewStream: mediaStream()
			? new MediaStream(mediaStream()?.getVideoTracks())
			: null,
		...
		clearBlobUrl: () => {
			...
			if (typeof mediaBlobURL === "string") URL.revokeObjectURL(mediaBlobURL)
...
export const SolidMediaRecorder = (props: MediaRecorderProps) =>
	props.render(useSolidMediaRecorder(props))
