...
export const Make_a_OutlineCircle_Around_Icon = (props: SvgOptions) => {
	...
	return (
		<svg
			...
			{...others}
		>
			{local.fill !== 'transparent' ? <circle cx="12" cy="12" r="12" fill={local.fill || "white"} stroke="none" stroke-width="2"></circle> : null}
		...
		>
			<span ref={(el: HTMLSpanElement) => (playGroupRef = el)} class="relative">
				...
				<Dynamic component={local.icons[0].hovered}
					...
					width={styles.width}
					height={styles.height}
					class="border group-active:scale-[1.03] group-focus:border rounded-full rounded-full absolute opacity-0 group-hover:opacity-100 -translate-y-full transition-[transform,opacity] duration-200"
				/>
			</span>
		...
		>
			<span class="relative">
				...
				<Dynamic component={local.icon.hovered}
					...
					width={styles.width}
					height={styles.height}
					class="group-active:scale-[1.03] group-focus:border rounded-full rounded-full absolute opacity-0 group-hover:opacity-100 -translate-y-full transition-[transform,opacity] duration-200"
				/>
			</span>
		</button>
	);
