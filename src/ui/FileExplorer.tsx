...
import { Remove, Plus } from "../assets/icons";
...
export const RenamingHandler = (
	...
	if (el !== null) {
		...
		values = document.getElementById(details.file_id)?.getBoundingClientRect() || new DOMRect();
	} else {
		values = document.getElementById(details.folder_id)?.getBoundingClientRect() || new DOMRect();
	}
			...
			{renamingState().folder_id ? (
				<InputField
					...
					onDone={async () => {
						...
						if (file_id !== "") {
							await renameFile({
                                folder_id, file_id, name:inputValue()
                            });
				...
				{(folder, index) => {
					...
					return (
						<div
							...
							<div
								class={classNames(
									folder().isopen ? "bg-white text-slate-800" : "bg-slate-500 text-slate-100",
                                ...
                                <button
                                        ...
                                        onClick={async () => {
                                            ...
                                            if(( folder.length === 0 || animationState() === 'expanded' || animationState() === 'added' ) && animationState() !== 'adding')  {
                                                let height = getHeight(panelRef);
                                                console.log(height)
                                                setPrevHeight(height);
                                                let file_id = await addFile({
                                                    folder_id:folder().id, name:"Files"
                                                });
                                ...
                                <button id="delete-for-folder-button" onClick={async () => {
									...
									<Index each={folder().files}>
										{(file, i) => {
											...
											return (
												<li
													ref={fileRef}
													class={classNames('flex flex-col',i === folder().files.length - 1 ? "pt-3 pb-1" : "pt-3")}>
													<div
														class={classNames(
															"flex px-2 rounded-lg drop-shadow-sm transition-colors",
															file().isactive ? "bg-cyan-500 text-white" : "bg-white text-slate-600"
														)}>
														<button
															class="w-full font-mono flex items-center"
															// Header
